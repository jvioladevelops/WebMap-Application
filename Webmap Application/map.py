...
map = folium.Map(location=[30, -90], zoom_start=6, tiles = "Stamen Terrain")
...
for lt, ln, el in zip(lat, lon, elev):
    fgv.add_child(folium.CircleMarker(location=[lt, ln], radius = 6, popup=folium.Popup(str(el)+" m",parse_html=True), fill_color=color_producer(el), color='grey', fill_opacity=0.7))
